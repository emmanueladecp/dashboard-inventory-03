...
export function RawMaterialsTable() {
  ...
  useEffect(() => {
    const filtered = rawMaterials.filter(material =>
      ...
      material.master_areas.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      material.erp_id.toString().includes(searchTerm)
    );
      ...
      <CardContent>
        {filteredMaterials.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">
              {searchTerm ? 'Try adjusting your search terms' : 'No materials have been added yet'}
            </p>
          </div>
        ) : (
                ...
                {filteredMaterials.map((material) => {
                  ...
                  return (
                    <TableRow key={material.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>{material.erp_id}</TableCell>
                      <TableCell className="font-mono">{material.current_stock}</TableCell>
                      <TableCell>{material.unit}</TableCell>
                      <TableCell>
                        <div className="flex flex-col">
                          <span className="font-medium">{material.master_areas.name}</span>
                          <span className="text-xs text-gray-500">ID: {material.master_areas.erp_id}</span>
                        </div>
                      </TableCell>
                      <TableCell>
